...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, email, plan, daily_usage, total_scripts)
  VALUES (
    NEW.id,
    NEW.email,
...
-- Create function to reset daily usage (for cron job)
CREATE OR REPLACE FUNCTION public.reset_daily_usage()
RETURNS void AS $$
BEGIN
  UPDATE public.profiles SET daily_usage = 0;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Insert default profiles for existing users (if any)
INSERT INTO public.profiles (id, email, plan, daily_usage, total_scripts)
SELECT 
  id, 
  email, 
